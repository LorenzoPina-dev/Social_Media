...
const TEST_DATABASE_URL =
  ...
  DATABASE_URL.replace(/\/([^/?]+)(\?|$)/, '/auth_test_db$2');
...
// __dirname in produzione = dist/scripts → le migrations stanno in dist/migrations
// __dirname in sviluppo   = scripts      → le migrations stanno in migrations
const MIGRATIONS_DIR = IS_COMPILED
  ? path.join(__dirname, '../migrations')   // dist/migrations
  : path.join(__dirname, '../migrations');  // migrations (stessa struttura)
