#!/usr/bin/env ts-node
/**
 * Database Initialization Script — user-service
 *
 * Crea il DB principale (user_db) E il DB di test (user_test_db),
...
const TEST_DATABASE_URL =
  ...
  DATABASE_URL.replace(/\/([^/?]+)(\?|$)/, '/user_test_db$2');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!testOnly) log('  user_db      → pronto per sviluppo');
    log('  user_test_db → pronto per i test (npm test)');
